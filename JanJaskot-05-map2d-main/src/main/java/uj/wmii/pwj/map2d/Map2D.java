...
public interface Map2D<R, C, V> {
    ...
    static <R,C,V> Map2D<R,C,V> createInstance() {
        return new Mapka<R,C,V>();
    }
}
class Mapka<R,C,V> implements Map2D
{
    int size=0;
    public Map<R,Map<C,V>> p_map;
    Mapka()
    {
        size=0;
        p_map= new HashMap<R,Map<C,V>>();
    }
    ...
    public Object put(Object rowKey, Object columnKey, Object value) {
        ...
        if(p_map.containsKey(rowKey))
        {
            t_map=p_map.get(rowKey);
        }
        else
        {
            t_map=new HashMap<C,V>();
            p_map.put((R)rowKey,t_map);
        }
        if(t_map.containsKey(columnKey))
        {
            Object l_toRet=t_map.get(columnKey);
            t_map.put((C)columnKey,(V)value);
            return l_toRet;
        }
        else
        {
    ...
    public Object remove(Object rowKey, Object columnKey) {
        ...
        if(p_map.containsKey(rowKey))
        {
            ...
            {
                size--;
                Object l_toRet=(Object) t_map.get(columnKey);
                t_map.remove(columnKey);
                if(t_map.size()==0)
                {
                    p_map.remove(rowKey);
                }
    ...
    public Map rowView(Object rowKey) {
        ...
        if(p_map.containsKey(rowKey)) {
            t_map=p_map.get(rowKey);
            Map<C,V> toRet=new HashMap<C,V>();
            for(var x:t_map.keySet())
            {
                toRet.put(x,t_map.get(x));
            }
            return toRet;
        }
        return null;
    ...
    public Map columnView(Object columnKey) {
        ...
        Map<R,V> toRet=new HashMap<R,V>();
        for(var x: p_map.keySet())
        {
            t_map=p_map.get(x);
            for(var i:t_map.keySet())
            {
                if(i.equals(columnKey))
                {
                    toRet.put(x,t_map.get(i));
                }
            }
            return toRet;
        }
    ...
    public boolean hasValue(Object value) {
        for (var x : p_map.keySet())
        {
            Map<C,V> t_map=p_map.get(x);
            if(t_map.containsValue(value))
            {
                return true;
            }
        }
    ...
    public boolean hasKey(Object rowKey, Object columnKey) {
        ...
        if(p_map.containsKey(rowKey))
        {
            Map<C,V> t_map=p_map.get(rowKey);
            if(t_map.containsKey(columnKey))
            {
                return true;
            }
    ...
    public Map rowMapView() {
        Map<R,Map<C,V>> mp= new HashMap<R,Map<C,V>>(p_map);
        return  mp;
    }

    @Override
    public Map columnMapView() {
        Map<C,Map<R,V>> mp= new HashMap<C,Map<R,V>>();
        for (var x: p_map.keySet())
        {
            Map<C,V> t_map=p_map.get(x);
            for (var y: t_map.keySet())
            {
                Map<R,V> t_map2;
                if(mp.containsKey(y))
                {
                ...
                {
                    t_map2= new HashMap<R,V>();
                    mp.put(y,t_map2);
                }
                t_map2.put(x,t_map.get(y));
            }
        }
        return mp;
    }
    ...
    public Map2D fillMapFromColumn(Map target, Object columnKey) {
        for(var x:p_map.keySet())
        {
            ...
            if(t_map.containsKey(columnKey))
            {
                target.put(x,t_map.get(columnKey));
            }
        }
        return this;
    }
    ...
    {
        Map<R,Map<C,V>> mapFromSource= source.rowMapView();
        for (var x:mapFromSource.keySet())
        {
            Map<C,V> t_map=mapFromSource.get(x);
            for (var y : t_map.keySet())
            {
                this.put(x,y,t_map.get(y));
            }
        }
        return this;
    }
    ...
    public Map2D putAllToRow(Map source, Object rowKey) {
       if(rowKey!=null)
       {
            for (var x:source.keySet())
            {
                this.put(rowKey,x,source.get(x));
            }
       }
       return this;
    }
    ...
    public Map2D putAllToColumn(Map source, Object columnKey) {
        if(columnKey!=null)
        {
            for (var x:source.keySet())
            {
                this.put(x,columnKey,source.get(x));
            }
        }
        return this;
    }
    ...
    public Map2D copyWithConversion(Function rowFunction, Function columnFunction, Function valueFunction)
    {
        ...
        Mapka<R,C,V> mapek= new Mapka<R,C,V>();
        for (var x:p_map.keySet())
        {
            Map<C,V> t_map=p_map.get(x);
            for(var y:t_map.keySet())
            {
                V value=t_map.get(y);
                R tempR=(R)rowFunction.apply(x);
                C tempC=(C)columnFunction.apply(y);
                V tempV=(V)valueFunction.apply(value);
                mapek.put(tempR,tempC,tempV);
            }
        }

        this.p_map=mapek.p_map;
        return this;
    }
}
